...
class ClientController extends Controller
{
    ...
    public function details(Request $request)
    {
        ...
        if ($request->client_type == 'Personal') {
            ...
            $client = Contract::active()->where('group_name', $request->group_name)->latest('date')->first();
        } else {
            return response()->json([
                'status' => false,
        ...
        return response()->json([
            ...
            'document' => $client->client_type == 'Personal' ? $client->document : $client->group_name,
            'name' => $client->name,
            'phone' => $client->phone,
    ...
    //Edicion de persona individual del grupo
    public function edit(Request $request)
    {
        if ($request->client_type === 'Personal') {
            ...
            if (!$client) return response()->json(['status' => false, 'error' => 'Cliente no encontrado'], 404);
        ...
        if ($request->client_type === 'Grupo') {
            ...
            if (!$contract) return response()->json(['status' => false, 'error' => 'Grupo no encontrado'], 404);

            $people = $contract->people ? json_decode($contract->people, true) : [];
            // ✅ buscar por document, no por id
            $person = collect($people)->firstWhere('document', $request->person_document);

            if (!$person) return response()->json(['status' => false, 'error' => 'Integrante no encontrado'], 404);
        ...
        return response()->json(['status' => false, 'error' => 'Tipo inválido'], 422);
    }
    ...
    public function updatePerson(Request $request)
    {
        if ($request->client_type == 'Personal') {
            ...
            return response()->json(['status' => true, 'message' => 'Cliente actualizado correctamente']);
        }
        ...
        $people = array_map(function ($person) use ($request) {
            if ($person['document'] == $request->person_document) {
        ...
        try {
            ...
            $contract->save();
            return response()->json(['status' => true, 'message' => 'Persona actualizada correctamente']);
        } catch (\Exception $e) {
